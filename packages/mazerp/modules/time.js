...
setInterval(() => {
    ...
    for(var p = 0; p < variables.PlayerInfo.length; p++) {
        ...
        if(variables.PlayerInfo[p].pFaction != undefined) {
            ...
            if(variables.PlayerInfo[p].iEmploymentTime >= 5) { // If time is 5 minutes, give player new employment tick, reset time for tick and add the payment he receives for that tick to the payment var.
                ...
                variables.PlayerInfo[p].iFactionPay += variables.PlayerInfo[p].pFaction.payPerTick;
            }
        }
    }
    ...
    if(minutes == 40) {
        for(var p = 0; p < variables.PlayerInfo.length; p++) {
                let Payment = variables.PlayerInfo[p].iFactionPay;
                ...
                variables.PlayerInfo[p].iFactionPay = 0;
        }
    }
}, 60000);